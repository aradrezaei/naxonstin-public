...
<div className="flex items-center gap-5">
  ...
  <Link href="/" className="flex items-center gap-5">
    ...
    {/* عنوان — فقط تو دسکتاپ */}
    <span className={`hidden lg:block text-2xl font-black tracking-tight ${scrolled ? "text-gray-900 dark:text-white" : "text-gray-900 dark:text-white"}`}>
      آموزشگاه نخستین
    </span>
  </Link>
</div>
            ...
            {/* منوی دسکتاپ */}
            <nav className={`hidden lg:flex items-center gap-1  font-medium ${scrolled ? "text-gray-700 dark:text-gray-300" : "text-gray-900 dark:text-white/90"}`}>
              {navItems.map(item => (
                <Link key={item.href} href={item.href} className="px-6 py-4 hover:bg-white/10 dark:hover:bg-white/5 rounded-xl transition-all">
                  {item.label}
                </Link>
              ))}
              <div className="relative group">
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-96 bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 ease-out translate-y-4 group-hover:translate-y-0">
                  <div className="p-8 grid grid-cols-2 gap-5">
                    {categories.map(cat => (
                      <Link key={cat} href="/categories" className="py-4 px-5 hover:bg-gradient-to-r hover:from-purple-500 hover:to-pink-500 hover:text-white rounded-2xl transition-all text-gray-700 dark:text-gray-300">
                        {cat}
                      </Link>
                    ))}
                  </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Link href="/login" className="hidden lg:block px-8 py-3.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl font-bold transition-all">
                ورود | ثبت‌نام
              </Link>
            </div>
          </div>
        ...
        <div className="p-6 space-y-1 overflow-y-auto h-full pb-32">
          ...
          <details className="group">
            ...
            <div className="py-3 space-y-2 pl-6">
              {categories.map(cat => (
                <Link key={cat} href="/categories" className="block py-2.5 text-purple-600 dark:text-purple-400 font-medium">
                  {cat}
                </Link>
              ))}
            </div>
